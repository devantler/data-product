...
public class EntitiesGenerator : GeneratorBase
{
    public override void Generate(SourceProductionContext context, Compilation compilation, IConfiguration configuration)
    {
        ...
        var @namespace = $"{compilation.AssemblyName}.Entities";

        foreach (var schema in schemas)
        {
            string source = ModelGenerator.GenerateModel(@namespace, schema, ModelType.Entity);
            context.AddSource($"{schema.Name}Entity.cs", SourceText.From(source, Encoding.UTF8));
        }
    }
}
