using Devantler.DataMesh.DataProduct.Features.DataStores.Entities;

namespace Devantler.DataMesh.DataProduct.Features.DataStores.Base;

public interface IRepository<T> where T : IEntity
{
    Task<Guid> Create(T entity, CancellationToken cancellationToken = default);
    Task CreateBulk(IEnumerable<T> entities, CancellationToken cancellationToken = default);
    Task<T> Read(Guid id, CancellationToken cancellationToken = default);
    Task<IEnumerable<T>> ReadBulk(IEnumerable<Guid> ids, CancellationToken cancellationToken = default);
    Task<IEnumerable<T>> ReadPaged(int page, int pageSize, CancellationToken cancellationToken = default);
    Task Update(T entity, CancellationToken cancellationToken = default);
    Task UpdateBulk(IEnumerable<T> entities, CancellationToken cancellationToken = default);
    Task Delete(Guid id, CancellationToken cancellationToken = default);
    Task DeleteBulk(IEnumerable<Guid> ids, CancellationToken cancellationToken = default);
    Task<IEnumerable<T>> Query(string query, CancellationToken cancellationToken = default);
}
